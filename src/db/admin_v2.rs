...
fn seed_initial_owner(transaction: &Transaction<'_>, owner_user_id: i64) -> rusqlite::Result<()> {
    let owner_is_active: Result<i64, _> = transaction.query_row(
        "SELECT is_active FROM users WHERE id = ?1",
        params![owner_user_id],
        |row| row.get(0),
    );

    match owner_is_active {
        Ok(1) => {}
        Ok(_) => return Err(rusqlite::Error::QueryReturnedNoRows),
        Err(rusqlite::Error::QueryReturnedNoRows) => {
            let now = chrono::Utc::now().timestamp();
            if transaction
                .execute(
                    "INSERT OR IGNORE INTO users (id, created_at, updated_at, is_active, telegram_id)
                     VALUES (?1, ?2, ?2, 1, NULL)",
                    params![owner_user_id, now],
                )
                .is_err()
            {
                transaction.execute(
                    "INSERT OR IGNORE INTO users (id, is_active) VALUES (?1, 1)",
                    params![owner_user_id],
                )?;
            }
            let _ = transaction.execute(
                ...
                params![format!("user:{owner_user_id}"), owner_user_id, now],
            );
        }
    ...
    transaction.execute(
        "INSERT INTO admin_assignments (
         ...
         VALUES (
            ...
            'active',
            ?1,
            'Первоначальный владелец GRABIT',
...
mod tests {
    ...
    #[test]
    fn migration_creates_placeholder_owner_when_missing() {
