...
fn resolve_search_city_geo_ids(
    db: &rusqlite::Connection,
    q: &str,
) -> BTreeMap<(usize, usize, usize), i64> {
    ...
    for (ci, (_, countries)) in world_data.iter().enumerate() {
        for (si, (_, cities)) in countries.iter().enumerate() {
...
pub async fn app_search(
    ...
    let mut city_geo_ids: BTreeMap<(usize, usize, usize), i64> = BTreeMap::new();
    ...
    if !q.is_empty() || kind.is_some() || explicit_rubric.is_some() || city_id.is_some() {
        ...
        city_geo_ids = resolve_search_city_geo_ids(&db, q);
